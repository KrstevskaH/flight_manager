...
{
    $aircrafts = $this->getAllAircraft(); 
    return view('aircraft.compare', compact('aircrafts'));
}


public function compare(Request $request)
{
    $selected = $request->aircrafts; 

    if (!$selected || count($selected) != 2) {
        return back()->with('error', 'Please select exactly 2 aircraft.');
    }

    $allAircrafts = collect($this->getAllAircraft());
    $aircrafts = $allAircrafts->whereIn('id', $selected)->values()->all(); 

    return view('aircraft.compare', compact('aircrafts'));
}
private function getAllAircraft()
{
    return [
        [
            'id' => 1,
